  ...
  return (
    ...
    >
      ...
      <div style={{ paddingTop: 32, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 12 }}>
        <div
          style={{
            ...
            boxShadow: '0 10px 28px rgba(208,120,80,0.32)',
          }}
        >
          <span style={{ fontSize: 28 }}>💰</span>
        </div>
        <p style={{ fontWeight: 800, fontSize: 22, color: '#1A1410', letterSpacing: '-0.01em' }}>
          SpendWise
        </p>
      </div>
      ...
      >
        ...
        {[
          ...
          <button
            key={t.label}
            type="button"
            onClick={() => { setIsSignIn(t.signIn); setError('') }}
            style={{
              position: 'relative',
              zIndex: 1,
              background: 'none',
      ...
      <AnimatePresence mode="wait">
        ...
        >
          <h1 style={{ fontWeight: 800, fontSize: 26, color: '#1A1410', letterSpacing: '-0.02em' }}>
      ...
      {/* Form */}
      <form onSubmit={handleSubmit} style={{ marginTop: 18, display: 'flex', flexDirection: 'column', gap: 12, flex: 1 }}>
        {!isSignIn && (
        ...
        <Field icon={<Lock size={16} color="#A8998A" />}>
          ...
          <button
            ...
            style={{
              ...
              background: 'none', border: 'none', cursor: 'pointer', color: '#A8998A', padding: 4,
            }}
        ...
        <AnimatePresence>
          {error && (
            <motion.p
              ...
              animate={{ opacity: 1 }}
              exit={{ opacity: 0 }}
              style={{ fontSize: 12, color: '#D03C3C', paddingLeft: 4, fontWeight: 500 }}
            >
              {error}
            </motion.p>
          )}
...
function Field({ icon, children }: { icon: React.ReactNode; children: React.ReactNode }) {
  return (
    ...
    >
      <span style={{ paddingLeft: 14, display: 'flex', alignItems: 'center' }}>{icon}</span>
      {children}
    </div>
  )
}
